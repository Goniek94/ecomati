...
export default function ProductDetailClient({
  ...
  const getCurrentStock = () => {
    ...
    return 999; // Default high stock if no variants
  };
  ...
  const getCurrentPrice = () => {
    ...
    return product.price; // Default price if no variants
  };
    ...
    <main className="min-h-screen bg-[#F6F5EE] text-[#1F2A14]">
      ...
      <div className="max-w-[1800px] mx-auto px-6 md:px-12 pt-32 pb-12">
        ...
        <div className="flex flex-col lg:flex-row gap-12 xl:gap-24 mb-24 items-start">
          {/* LEWA STRONA */}
          <div className="lg:w-1/2 relative w-full flex flex-col gap-8">
            <div className="h-[50vh] lg:h-[80vh] w-full bg-[#EBE9DE] rounded-[2rem] overflow-hidden border border-[#1F2A14]/5 shadow-sm">
              <motion.div
            ...
            {/* DŁUGI OPIS POD ZDJĘCIEM - max 1000 znaków */}
            {product.longDesc && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.3 }}
                className="bg-white p-8 rounded-[2rem] border border-[#1F2A14]/5 shadow-sm"
              >
                ...
                <p className="text-[#1F2A14]/80 text-base leading-relaxed font-light">
                  {product.longDesc.length > 1000
                    ? `${product.longDesc.substring(0, 1000)}...`
                    : product.longDesc}
                </p>
              </motion.div>
            )}
    ...
    <div className="border-b border-[#1F2A14]/5 last:border-0">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="pb-6 text-[#6B705C] text-sm leading-relaxed max-w-lg">
